# Backup Postgres (PostgreSQL) databases to an S3 bucket

postgres-s3-backup is inspired by the popular [postgres-backup-s3](https://github.com/schickling/dockerfiles/tree/master/postgres-backup-s3) project.

## Usage

Docker:
```sh
$ docker run -e S3_ACCESS_KEY_ID=key -e S3_SECRET_ACCESS_KEY=secret -e S3_BUCKET=my-bucket -e S3_PREFIX=backup -e POSTGRES_DATABASE=dbname -e POSTGRES_USER=user -e POSTGRES_PASSWORD=password -e POSTGRES_HOST=localhost thilinaperera/postgres-s3-backup
```
...
pgbackups3:
  image: thilinaperera/postgres-s3-backup:16
  depends_on:
    - postgres
  links:
    - postgres
